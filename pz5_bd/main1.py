...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.tab6 = self.create_tab6()  # Вкладка для редактирования файла бота
        ...
        self.tabs.addTab(self.tab6, "Редактировать файл бота")

        # Вкладка 6: Редактирование файла бота

    def create_tab6(self):
        tab = QWidget()
        layout = QVBoxLayout()

        # Поле для редактирования файла
        self.bot_file_edit = QTextEdit()
        self.bot_file_label = QLabel("Файл бота не выбран.")
        self.open_bot_file_button = QPushButton("Открыть файл бота")
        self.save_bot_file_button = QPushButton("Сохранить изменения")

        # Привязываем кнопки к функциям
        self.open_bot_file_button.clicked.connect(self.open_bot_file)
        self.save_bot_file_button.clicked.connect(self.save_bot_file)

        # Добавляем элементы в компоновку
        layout.addWidget(self.bot_file_label)
        layout.addWidget(self.bot_file_edit)
        layout.addWidget(self.open_bot_file_button)
        layout.addWidget(self.save_bot_file_button)
    ...
    def open_bot_file(self):
        file_name, _ = QFileDialog.getOpenFileName(self, "Открыть файл бота", "", "")
        if file_name:
            self.current_bot_file = file_name
            with open(file_name, 'r', encoding='utf-8') as file:
                self.bot_file_edit.setText(file.read())
            self.bot_file_label.setText(f"Файл открыт: {file_name}")

        # Сохранить изменения в файле бота

    def save_bot_file(self):
        if hasattr(self, 'current_bot_file') and self.current_bot_file:
            with open(self.current_bot_file, 'w', encoding='utf-8') as file:
                file.write(self.bot_file_edit.toPlainText())
            self.bot_file_label.setText(f"Изменения сохранены: {self.current_bot_file}")
        else:
            self.bot_file_label.setText("Сначала откройте файл бота.")
